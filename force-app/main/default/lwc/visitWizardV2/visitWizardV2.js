...
import getVisitRecordTypes from '@salesforce/apex/VisitWizardController.getVisitRecordTypes';
...
import saveVisit from '@salesforce/apex/VisitWizardController.saveVisit';

const STEP_RECORD_TYPE = 1;
const STEP_VISIT_DETAIL = 2;
const STEP_VISITORS = 3;
const STEP_VISITED_PARTIES = 4;
const COMPONENT_SELECTOR = 'c-visit-wizard-v2';
...
export default class VisitWizard extends NavigationMixin(LightningElement) {
    ...
    async handleRecordTypeSelect(event) {
        ...
        try {
            const fields = await getVisitFieldMetadata({ recordTypeId: this.selectedRecordTypeId });
    ...
    handleVisitorCandidateToggle(event) {
        const userId = event.target.dataset.id;
        const checked = event.target.checked;
        this.visitorCandidates = this.visitorCandidates.map((visitor) => {
            if (visitor.userId === userId && !visitor.locked) {
                return { ...visitor, selected: checked };
            }
            return visitor;
        });

        if (checked) {
            const visitor = this.visitorCandidates.find((candidate) => candidate.userId === userId);
            this.addVisitor(visitor);
        } else {
            this.selectedVisitors = this.selectedVisitors.filter((visitor) => visitor.userId !== userId || visitor.locked);
        }
    }
    ...
    addVisitor(visitor) {
        ...
        this.selectedVisitors = [...this.selectedVisitors, { ...visitor, selected: true }];
